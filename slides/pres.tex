...
~                                                                                       
    ...
    \begin{frame}[t,fragile]{Killing a running process}
        \begin{columns}[T]
            \column{0.5\textwidth}
	    \begin{itemize}
		    \item Every process has an ID an Linux. 
		    \item View processes with \texttt{ps -e}.
...
sudo dnf update
    ...
    \begin{frame}[t,fragile]{Github}
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{itemize}
		    \item www.github.com has a \emph{lot} of software
		    \item All open source
		    \item This is just like software you download under Windows, so be careful!
		    \item If you find something you're interested in: \texttt{git clone link}
            \end{itemize}
            \column{0.5\textwidth}
            \begin{bashenv}[\tiny]
[luke@host ~]$ git clone https://github.com/lampepfl/dotty.g8
Cloning into 'dotty.g8'...
done.
[luke@host ~]$ ls
§\textcolor{dirblue}{dotty.g8}§
            \end{bashenv}
        \end{columns}
    \end{frame}

...
\subsection{Globbing (Wildcards)} Globbing is a useful tool to work with files
that share a pattern. For example, to refer to all \texttt{jpg} images in a
directory, you can use \texttt{*.jpg}. The \texttt{*} means `any number of any
characters', so this pattern will give you all files that end with
\texttt{.jpg}. Similarly, \texttt{dir/*} will give you \emph{all} files in
\texttt{dir}, \emph{except} hidden files.
...
\hintbox{%
If you don't have an ETH account, use \texttt{pterodactyl@vsos.ethz.ch}
with password \texttt{BoredHacker} for the following exercises instead.}

...
\begin{table}[H]
    ...
    \begin{tcolorbox}[%
        ...
        \texttt{rm -r DIR} &  Remove a directory and its contents.
            {\color{exclamred}Attention! There is no trashcan on the command line!
...
\begin{exercisebox}{Cat tricks}
	Type the command \texttt{cat -}. This starts \texttt{cat} to read from \texttt{stdin}. Enter some text and observe what happens when you press \keys{Enter}. \texttt{cat} will just keep reading until the end-of-file character, so you can enter multiple lines of text. When you are done, terminate the input as described above.
\end{exercisebox}
